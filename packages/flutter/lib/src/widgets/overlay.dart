...
class OverlayState extends State<Overlay> with TickerProviderStateMixin {
  ...
  void insertAll(Iterable<OverlayEntry> entries, { OverlayEntry below, OverlayEntry above }) {
    ...
    assert(above == null || (above._overlay == this && _entries.contains(above)));
    assert(below == null || (below._overlay == this && _entries.contains(below)));
    assert(entries.every((OverlayEntry entry) => !_entries.contains(entry)), 'One or more of the specified entries are already present in the Overlay.');
    assert(entries.every((OverlayEntry entry) => entry._overlay == null), 'One or more of the specified entries are already present in another Overlay.');
    if (entries.isEmpty)
      return;
  ...
  void rearrange(Iterable<OverlayEntry> newEntries, { OverlayEntry below, OverlayEntry above }) {
    ...
    assert(newEntriesList.every((OverlayEntry entry) => _entries.indexOf(entry) == _entries.lastIndexOf(entry), 'One or more of the specified entries are specified multiple times.'));
    if (newEntriesList.isEmpty)
      return;
    if (listEquals(_entries, newEntriesList))
      return;
    final LinkedHashSet<OverlayEntry> old = new LinkedHashSet<OverlayEntry>.from(_entries);
